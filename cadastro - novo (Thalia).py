...
def database():
    ...
    with conn:
        cursor = conn.cursor()
    cursor.execute('CREATE TABLE IF NOT EXISTS Student (Fullname TEXT, Email TEXT, Gender TEXT, etc)')
    cursor.execute('INSERT INTO Student (Fullname, Email, Gender, country, Programming) VALUES (?,?,?,?,?)',(name1, email, gender, country, prog,))
    conn.commit()

label_0 = Label(root, text = "Registration Planner", width = 20, font =('bold', 20))
